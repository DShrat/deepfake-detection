...
modelR = tf.keras.Model(
    inputs=resnet50.input, outputs=resnet50.get_layer("conv4_block5_out").output
)

xception = tf.keras.applications.xception.Xception(
    include_top=False, weights="imagenet"
)
modelX = tf.keras.Model(
    inputs=xception.input, outputs=xception.get_layer("conv2d_2").output
)

vgg19 = tf.keras.applications.vgg19.VGG19(include_top=False, weights="imagenet")
modelV = tf.keras.Model(
...
def try_predict_resnet(filepath):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        predicted_labels = sess.run(
            predictions, feed_dict={X: combined_feature.reshape(1, 14, 14, 1)}
        )
...
# Route to predict if the image is real or deepfake
@app.post("/predict")
async def predict(file: UploadFile = File(...)):
    if not (file.filename.endswith(".jpg") or file.filename.endswith(".jpeg")):
        return {"error": "invalid image type, must be jpg or jpeg"}
